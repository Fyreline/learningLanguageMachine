"""The Mishka Hub identity client — docs/AUTH.md §2-3.

Michi holds no credential store of its own. At login it forwards the
...
trivially and a future standalone mode would swap one class (docs/AUTH.md §2
note).
"""
...
class MishkaIdentityClient:
    def __init__(self, base_url: str, timeout: float = 5.0) -> None:
        ...
        if parsed.scheme == "http" and not is_loopback:
            raise ValueError(
                "MICHI_MISHKA_BASE_URL must be HTTPS or loopback "
                f"(got {base_url!r}) — docs/AUTH.md §4"
            )
    ...
    async def verify(self, email: str, password: str) -> IdentityUser:
        ...
        fixed message, never the request body (docs/AUTH.md §4).
        """
        try:
    ...
    async def _best_effort_logout(self, client: httpx.AsyncClient, refresh_token: str) -> None:
        """Keeps Mishka's refresh_tokens table tidy (docs/AUTH.md §2 note).
        Never raises — a failure here must never fail the Michi login."""
        try:
    ...
    async def ping(self) -> bool:
        """1s-timeout reachability probe used by routers/health.py (cached there 60s)."""
        try:
