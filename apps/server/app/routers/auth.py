"""Login/session endpoints — docs/AUTH.md §2-3, docs/API.md "Auth".
...
GET  /api/auth/me       the authenticated user's own profile + settings
PUT  /api/auth/settings merge-patch of the user's settings_json
...
# --- simple in-process login rate limit (docs/AUTH.md §2: "same 5-failures/
# 15-min/IP deque as Mishka"), in front of the identity proxy call so a brute
# force can't use Michi to hammer Mishka. ---
...
async def login(body: LoginBody, request: Request, session: Session = Depends(get_session)) -> TokenPair:
    ...
    if not settings.auth_configured:
        raise MichiHTTPException(
            status_code=503,
            detail="Login is not configured on this server (MICHI_JWT_SECRET unset).",
    ...
    # Upsert local users row keyed by lower(email) — display_name/mishka_user_id
    # refreshed on every login, so renames on the Mishka Hub side follow too
    # (docs/AUTH.md §2).
    ...
    if user is None:
        user = User(
            ...
            mishka_user_id=identity_user.id,
        )
        session.add(user)
    else:
        user.display_name = identity_user.display_name
        user.mishka_user_id = identity_user.id
    session.commit()
...
async def refresh(
    ...
    if row is None:
        raise MichiHTTPException(status_code=401, detail="Invalid refresh token", code="invalid_refresh_token")

    if row.revoked:
        # Reuse of an already-rotated-away token: theft tripwire — revoke
        # every refresh token this user holds, forcing a fresh login
        # everywhere (docs/AUTH.md §5). Mishka Hub sessions are untouched.
    ...
    if expires_at < datetime.now(timezone.utc):
        raise MichiHTTPException(status_code=401, detail="Refresh token expired", code="invalid_refresh_token")
...
async def me(user_id: int = Depends(current_user), session: Session = Depends(get_session)) -> dict:
    ...
    if user is None:
        raise MichiHTTPException(status_code=404, detail="User not found", code="not_found")
    return {
        "id": user.id,
        "email": user.email,
...
class SettingsPatchBody(BaseModel):
    """Every field optional — PUT /api/auth/settings is a merge-patch
    (docs/API.md), only the keys present in the request body are touched.
    Shape mirrors docs/DATA_MODEL.md users.settings_json."""
...
async def update_settings(
    ...
    current = json.loads(user.settings_json)
