...
Ported from Dev/MishkaHub/apps/server/app/security.py per docs/AUTH.md §3
step 1: the password-hashing functions and their password-hashing-library
import are deleted entirely — Michi never stores, hashes, or even sees a
hash of a password (there is no password-hashing dependency anywhere in
requirements.txt; that absence is the proof Michi holds no passwords, see
docs/AUTH.md §5).
"""
...
# token. Rotated on every use (docs/AUTH.md §3): the presented token is
# marked revoked and a new one issued in the same request.
# --------------------------------------------------------------------------
