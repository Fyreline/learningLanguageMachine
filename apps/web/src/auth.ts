// Session state for the household login (docs/AUTH.md). Port of Mishka
// Hub's apps/web/src/auth.ts (docs/AUTH.md §3.5): base URL defaults to
// Michi's own server (127.0.0.1:8100) and the localStorage key is renamed
// to `michi-refresh-token` — a distinct key from Mishka's `mishka-refresh-
// token`, since both SPAs may share `localhost` during dev and clobbering
// Mishka's session would be a rude bug.
//
...
const REFRESH_STORAGE_KEY = 'michi-refresh-token'
