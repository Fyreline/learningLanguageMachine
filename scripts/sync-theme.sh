...
# Also mirrors docs/HOUSEHOLD-DESIGN.md (the cross-app design language) to each sibling's docs/.
set -euo pipefail
SRC="$(cd "$(dirname "$0")/.." && pwd)/apps/web/src/theme.css"
DOC_SRC="$(cd "$(dirname "$0")/.." && pwd)/docs/HOUSEHOLD-DESIGN.md"
...
for APP_DIR in /Users/mack/Documents/Dev/MishkaHub /Users/mack/Documents/Dev/Japan_website /Users/mack/Documents/Dev/Finances /Users/mack/Documents/Dev/sukumo; do
  cp "$DOC_SRC" "$APP_DIR/docs/HOUSEHOLD-DESIGN.md"
  echo "synced: $DOC_SRC -> $APP_DIR/docs/HOUSEHOLD-DESIGN.md"
  diff -q "$DOC_SRC" "$APP_DIR/docs/HOUSEHOLD-DESIGN.md" && echo "in step ✓"
done
